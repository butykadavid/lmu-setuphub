...
async function getTelemetrySummary(
  ...
  return {
    ...
    carModel: data.selectedCar?.name || "",
  ...
  try {
    ...
    if (searchQuery) {
      summaries = summaries.filter((tel) => {
        const searchableText = `${tel.carModel || ""} ${extractMetadataValue(tel.telemetry?.metadata, "TrackName") || ""} ${extractMetadataValue(tel.telemetry?.metadata, "DriverName") || ""} ${tel.driverNote || ""}`
          .toLowerCase();
    ...
    if (trackFilter) {
      summaries = summaries.filter((tel) =>
        extractMetadataValue(tel.telemetry?.metadata, "TrackName")?.toLowerCase().includes(trackFilter)
      );
    }
